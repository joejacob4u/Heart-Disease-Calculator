 ...
 */
public class j48 extends Distribution implements OptionHandler, 
  Drawable, Matchable, Sourcable, WeightedInstancesHandler, Summarizable,
  AdditionalMeasureProducer {

  public j48(Distribution toMerge) {
		super(toMerge);
...
// To maintain the same version number after adding m_ClassAttribute
  ...
  public void buildClassifier(Instances instances) 
    ...
    if (m_binarySplits)
      modSelection = new BinC45ModelSelection(m_minNumObj, instances, m_binarySplits);
    else
      modSelection = new C45ModelSelection(m_minNumObj, instances, m_binarySplits);
    if (!m_reducedErrorPruning)
      m_root = new C45PruneableClassifierTree(modSelection, !m_unpruned, m_CF,
					    m_subtreeRaising, !m_noCleanup, m_binarySplits);
    else
      m_root = new PruneableClassifierTree(modSelection, !m_unpruned, m_numFolds,
					   !m_noCleanup, m_minNumObj);
  ...
  public static void main(String [] argv){
    ...
    try {
      System.out.println(Evaluation.evaluateModel((Classifier) new j48(null), argv));
    } catch (Exception e) {
